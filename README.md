...
- Before the activity begins, one person moves close to the camera for five clear, centered face samples over roughly four seconds. The oval guide says whether to move closer, hold still, look straight, center up, or leave only one person in view. Capture stays paused until four of the five HD-camera views agree on the same enrolled worker, or **Continue without face matching** is chosen.
...
This confirmation is presentation only. It does not change face descriptors, thresholds, roster selection, or the four-of-five agreement required by recognition; it makes an already-completed match apparent to the worker. Auto capture may initialize behind the confirmation, but the confirmation remains on top until its 1.8-second display interval ends. Skipped and unsuccessful scans do not show the success state.
...
Matched people are labelled with their enrolled worker IDs, such as **WORKER-007**, in the live overlay and saved photos. A person who has not matched the roster is labelled **UNMATCHED WORKER**; the numeric session track ID remains internal. A local session tracker follows position, predicted motion, scale, coarse body proportions, a small gallery of clothing-colour samples, and facial re-identification. MediaPipe supplies the face and eye positions; StampNote aligns a temporary face crop and a local face-recognition network turns it into a unit-normalized 128-value embedding. A separate `onboarding.html` flow checks seven spaced, consistent samples over about six seconds and stores the seven representative templates plus their centroid in Firestore; the recording page's five-view opening scan compares against that roster before capture begins. Both camera paths request 1920 × 1080 input, then fall back to the best resolution the device provides. The opening scan uses a focused face-only landmarker, so a face can fill the guide without requiring shoulders or hips to remain visible. Landmark geometry and lighting-normalized facial texture remain fallbacks while that model loads or when a face is too small to recognize reliably. A clear embedding mismatch takes priority over the weaker signals, preventing two people with similar face proportions or clothing from being merged. Whole-frame assignment prevents a locally convenient match from swapping the remaining people, while the sample galleries tolerate changes in expression, pose, lighting and partial re-entry at the edge. The badge says **face match on-device** only after the trained model has produced a usable live embedding.
...
Photo documents remain under `users/{uid}/photos/{photoId}`, while worker templates live in the team-wide `workers/{workerId}` roster. Firestore requires Google authentication but, by explicit project policy, any authenticated project user can read or write every document. Anonymous visitors remain blocked. No public image URL exists: the authenticated dashboard turns Firestore bytes into a temporary in-browser image URL. Open `admin.html` to browse photos grouped first by stamped location and then by date, and open `onboarding.html` to enroll, replace, or delete worker templates.
...
3. The watch photographs the scene on its own — every 30 seconds with a person in frame, every 120 seconds without. Public boxes state the enrolled worker ID, or `UNMATCHED WORKER` until a roster match is available. Raise both hands above your head to take one there and then.
...
6. Open the photo dashboard to browse by location and date or filter the recoverable AI review bin. Save kept photos out as files, or share a hand-picked image through the OS share sheet.
...
- `firebase.js` — Google Authentication plus Firestore photo and worker-template access
- `admin.html`, `admin.css`, `admin.js` — authenticated cloud photo dashboard
