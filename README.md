...
- Before the activity begins, one person moves close to the camera for two clear, centered face samples over roughly one second. The oval guide says whether to move closer, hold still, look straight, center up, or leave only one person in view. A continuous progress bar keeps the sample count out of the interface. Capture stays paused until both HD-camera views agree on the same enrolled worker; a missed match keeps scanning automatically, and **Continue without face matching** remains available.
...
This confirmation is presentation only. It does not change face descriptors, thresholds, roster selection, or the two-view agreement required by recognition; it makes an already-completed match apparent to the worker. Auto capture may initialize behind the confirmation, but the confirmation remains on top until its 1.8-second display interval ends. Skipped and still-retrying scans do not show the success state.
