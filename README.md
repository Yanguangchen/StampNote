...
Open **Photos & attendance** (`admin.html`) to browse photos alongside recent attendance grouped by street, recorded address, date, and time session. Open **Live tunnel** (`live-tunnel.html`) to watch any camera that is recording right now — the picture opens like a video call, without anyone accepting or rejecting it. An administrator can **Talk** so live microphone audio plays on Robotic control, or send a voice-message clip that plays on that device. After sign-in it auto-joins the first live recording. The recordings list hides behind an icon; each card has **Watch**. In landscape the page splits half robot control and half live stream; on a phone held upright the live picture keeps the screen until a **Robot IP address** is opened, then the control page fills most of the height. Opening that address also starts that session's camera. Open **Worker onboarding** (`onboarding.html`) to enroll, replace, or delete worker templates; the worker ID is issued from the typed name rather than entered by hand. **Worker photos** (`worker-photos.html`) is the field path that does not start the watch: take one photo or pick several, stamp a fresh GPS fix and the day's weather, run Gemini sanitization, then save locally and sync when signed in. **Robotic control** (`robotic-control.html`) is a dedicated full-page live camera for robot teleoperation: the stream starts when the page opens, with no MediaPipe, attendance, or auto capture, Live tunnel sharing when signed in, and `streamIncomingAudio` for live talk from Live tunnel. **Metrics** (`metrics.html`) plots three independent daily series — attendance taken, flags raised, and sessions created — over the last 7 or 30 days. **Operations AI** (`ai-dashboard.html`) answers questions about those loaded records.
...
- `src/services/live-tunnel.js` — WebRTC publisher/viewer with Firestore signaling, JPEG picture relay, live talk audio into the recording, and no call prompt
- `src/services/robot-control-url.js` — parses http(s) robot IP addresses for the Live tunnel iframe
...
- `robotic-control.html`, `robotic-control.css`, `robotic-control.js` — dedicated full-page live camera for robot controls; starts on page open, without MediaPipe; `StampNoteRoboticControl.streamIncomingAudio` plays live talk from Live tunnel
